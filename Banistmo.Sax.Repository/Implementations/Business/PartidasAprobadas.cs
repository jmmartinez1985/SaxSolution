                ...
                var resultComprobante = (from p in db.vi_PartidasAprobadas
                                         ...
                                         where (p.PA_STATUS_PARTIDA == aprobado
                                            || p.PA_STATUS_PARTIDA == anulado || p.PA_STATUS_PARTIDA == porAprobar)
                                             ...
                                             && ((p.PA_TIPO_CONCILIA == conciliacionAuto || p.PA_TIPO_CONCILIA == conciliacionManual || p.PA_TIPO_CONCILIA == 0) || (p.PA_TIPO_CONCILIA == conciliacionParcial && ( p.PA_IMPORTE_PENDIENTE != 0 || p.PA_IMPORTE_PENDIENTE !=null)))
