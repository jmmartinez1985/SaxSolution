...
namespace Banistmo.Sax.Repository.Implementations.Business
{
    ...
    public class RegistroControl : RepositoryBase<SAX_REGISTRO_CONTROL>, IRegistroControl
    {
        ...
        public string IsValidReferencia(string referencia, string empresa, string moneda, string cuenta_contable,string centro_costo, decimal monto_saldo, ref decimal monto, ref int tipo_error)
        {
            ...
            var value = newContext.ObjectContext.Database.SqlQuery<ReferenciaForker>("USP_BUSCAR_REFERENCIA @i_referencia,@i_empresa,@i_moneda,@i_cta_contable,@i_importe,@i_centro_costo", parameters.ToArray()).ToList();
            var res = value.FirstOrDefault();
            if (res.IMPORTE != null)
            {
                monto = Convert.ToDecimal(res.IMPORTE);
